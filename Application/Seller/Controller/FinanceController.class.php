        ...
        $withdrawals = $Model->where("id = $id")->find();
         ...
         if(IS_POST)
         {
             ...
             }else
             {
                 ...
                 $money = I('money', 0);
                 if ($type == 1) {
                     $store =  M('store')->where("store_id = ".STORE_ID." and store_money >= {$money}")->find();
                     if (! $store) {
                         $this->error('提现金额不足',U('add_edit_withdrawals'));
                         exit();
                     }
                     if (bccomp($money, $store['store_money'], 2) < 1) {
                         $remark = '商户提现';
                         $out_trade_no = get_ali_trans_sn();
                         $res = transferAliaop($out_trade_no, $data['account_bank'], $money, $data['account_name'], $remark);
                         ...
                         if($res['status'] == 1) {
                             ...
                             $Model->order_id = h_property_get($res['result'], 'order_id', "");
                             $withdrawals_id = $Model->add();
                             storeAccountLog(STORE_ID, ($money * -1), 0, $desc = '平台提现');
                             $remittance = array(
                                 'store_id' => STORE_ID,
                             ...
                             $this->success('支付宝提现成功', U('store_withdrawals'));
                             exit;
                         } else {
                             $this->error("提现失败:" . $res['msg']);
                             exit;
                         }
                     } else {
                         $this->error('提现金额不足',U('add_edit_withdrawals'));
