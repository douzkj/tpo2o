...
class GoodsController extends BaseController {
   ...
   /**
    ...
    public function addEditGoods(){
            ...
            if(($_GET['is_ajax'] == 1) && IS_POST)
            {
                ...
                if(!$Goods->create(NULL,$type))// 根据表单提交的POST数据创建数据对象
                {
                   ...
                   // C('TOKEN_ON',true);
                    ...
                    if (empty(I('shop_ids', []))) {
                        $this->ajaxReturn(json_encode(array('status' => -1,'msg'=> '适用门店不能为空')));
                    }
                    $cat_id3 = I('cat_id3',0);
                    ...
                    $GoodsLogic->saveGoodsShop($goods_id, I('shop_ids', []), STORE_ID); // 处理商品适用门店
            ...
            $shops = M('store_shops')->where(['store_id' => STORE_ID, 'state' => 1])->select();
            $goods_shops = M('goods_shop')->where(['goods_id' => $goods_id])->getField('shop_id', true);
            $this->assign('goods_shops', $goods_shops);
