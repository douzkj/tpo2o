...
class OrderCodeController extends BaseController {
    ...
    public function ajaxindex(){
        ...
        // 搜索条件 STORE_ID
        $condition = array('b.store_id'=>STORE_ID, "b.pay_status" => 1); // 商家id
        if($begin && $end){
            $condition['b.add_time'] = array('between',"$begin,$end");
        }
        if (I('order_sn')) {
            $condition['b.order_sn'] = I('order_sn');
        }
        if (I('is_checked') && trim(I('is_checked') != '')) {
            $condition['a.is_checked'] = I('is_checked');
        }

        $sort_order = I('order_by','DESC').' '.I('sort');
        $count = M('order_codes a')
            ->join('__ORDER__ b on a.order_id = b.order_id', "INNER")
            ->where($condition)
            ->where("(b.group_order_id <> 0 and b.group_status <> 2) or (b.group_order_id = 0) ")
            ->count();
        $Page  = new AjaxPage($count,20);
        $show = $Page->show();
        //获取订单列表
        $orderList = M('order_codes a')
            ->join('__ORDER__ b on a.order_id = b.order_id', "INNER")
            ->where($condition)
            ->where("(b.group_order_id <> 0 and b.group_status <> 2) or (b.group_order_id = 0) ")
            ->field('a.*, b.order_sn, b.consignee, b.mobile, b.add_time,b.group_order_id, b.group_status')
            ->limit("{$Page->firstRow},{$Page->listRows}")
            ->order($sort_order)->select();
//        $orderList = $orderLogic->getCodeList($condition,$sort_order,$Page->firstRow,$Page->listRows);
    ...
    {
        $code = M('order_codes')->where(['id' => intval(I('id')), 'is_checked' => 0])->find();
        if ($code) {
            $order = M('order')->where('order_id = '.$code['order_id'].' and pay_status=1 AND is_used !=1 AND (group_order_id = 0 OR (group_order_id <> 0 AND group_status = 1)) AND order_status in(0,1)')->find();
            if (!$order) {
                $this->error('订单状态不符或已消费完毕');
            }
            $seller = session('seller');
            $codes = M('order_codes')->where(['id' => $code['id'], 'is_checked' => 0])->save([
                'user_id' => $seller['seller_id'],
                'shop_id' => $seller['shop_id'],
                'check_at' => time(),
                'is_checked' => 1
            ]);
            if (M('order_codes')->where(['order_id' => $code['order_id'], 'is_checked' => 0])->count() == 0) {
                M('order')->where(['order_id' => $code['order_id']])->save([
                    'is_used' => 1,
                    'order_status' => 4
                ]);
            }
